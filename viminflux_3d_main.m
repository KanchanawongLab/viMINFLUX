...
dz=0.001;      % pixel size for z 
zpos=(-50:50).*dz;   %z coordinates to calculate
...
% TC2 and three phase shifted pattern
faiorb=2*pi/3;
...
% TC3 and three phase shifted pattern
faiorb=4*pi/3;
...
x0=0; y0=0; z=0;
fai=atan2(kxx1,kyy1);
for jj=1:1:length(zpos)   
   focpos=zpos(jj);
   for k=1:K
      fpupil=CTF1.*czt_getphasexyz(kxx1,kyy1,kz_c1,kz_c2,x0+pos_nm(k,1),y0+pos_nm(k,2),0,focpos);
      fmask1=exp(1i.*fai);
      ...
      beadpsf(:,:,jj,k)=psfabc;  % save beadpsf
...
nzmid=floor(size(zpos,2)/2)+1;
sumpsf=sum(sum(sum(beadpsf,1),2),4);
Imidpsf=sumpsf(1,1,nzmid);   
beadpsf=1.*beadpsf./Imidpsf;
    

% calculate the localization precision for 3D viMinflux
Nph=500; SBR=5;    
[sigma_crb, sigma_crbx, sigma_crby, sigma_crbz]= crbMinflux3D(beadpsf, Nph, SBR);

sigma_xy= (sigma_crbx+sigma_crby)/2; 
roix=floor(1000*L/2); midx=101; midz=51;

%plot the 2d precisions inside the scan range (-L/2: :L/2)
figure
imagesc(sigma_xy(midx-roix:midx+roix,midx-roix:midx+roix,midz).*sqrt(Nph),[0 120]);
title('lateral precision for 3D viminflux')

figure
imagesc(squeeze(sigma_crbz(midx-roix:midx+roix,midx-roix:midx+roix,midz)).*sqrt(Nph), [0 120]);
title('axial precision for 3D viminflux in the middle xy plane')

figure
imagesc(squeeze(sigma_crbz(midx-roix:midx+roix,midx,:)).*sqrt(Nph), [0 120]);
title('axial precision for 3D viminflux in the xz plane')


figure
plot(sigma_xy(midx-roix:midx+roix,midx,midz).*sqrt(Nph),'r')
hold on
plot(sigma_crbz(midx-roix:midx+roix,midx,midz).*sqrt(Nph),'g')
legend('\sigma_{xy}','\sigma_z')